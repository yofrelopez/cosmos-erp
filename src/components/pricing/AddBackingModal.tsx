...
export default function AddBackingModal({ open, onClose, onSuccess, companyId }: Props) {
  ...
  const onSubmit = async (data: BackingFormData) => {
    ...
    setLoading(true)
    try {
      const response = await fetch('/api/pricing/backings', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        toast.error(error.message || 'Error al crear el soporte')
      }
    } catch (error) {
